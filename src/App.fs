...
module App =
    ...
    let MechSim() =
        ...
        // Explicit típusokkal ellátott handler-ek → ez oldja meg a "too many arguments" hibát
        let handleLength (e: Event) : unit =
            let value = (e.target :?> HTMLInputElement).value |> float
            update forceUpdate (fun s -> { s with Length = value })

        let handleForce (e: Event) : unit =
            let value = (e.target :?> HTMLInputElement).value |> float
            update forceUpdate (fun s -> { s with Force = value })

        let setCantilever (_: Event) : unit =
            update forceUpdate (fun s -> { s with BeamType = Cantilever })

        let setSimplySupported (_: Event) : unit =
