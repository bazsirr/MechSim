...
module App =
    ...
    let MechSim() =
        ...
        // Event handler-ek külön definiálva → elkerüljük a template-ben lévő komplex lambda hibát
        let handleLengthChange (e: Event) =
            let value = (e.target :?> HTMLInputElement).value |> float
            update forceUpdate (fun s -> { s with Length = value })

        let handleForceChange (e: Event) =
            let value = (e.target :?> HTMLInputElement).value |> float
